/*
__author__ = 'lawtech'
__date__ = '2020/05/18 2:13 下午'
*/

package _432

type AllOne struct {
	data map[string]*LinkedListNode
	list *LinkedList
}

/** Initialize your data structure here. */
func Constructor() AllOne {
	return AllOne{
		data: make(map[string]*LinkedListNode),
		list: NewLinkedList(),
	}
}

/** Inserts a new key <Key> with value 1. Or increments an existing key by 1. */
func (ao *AllOne) Inc(key string) {
	if n, ok := ao.data[key]; ok {
		n.Val++
		ao.rightAdjust(n)
	} else {
		n := &LinkedListNode{Key: key, Val: 1}
		ao.list.LPush(n)
		ao.data[key] = n
	}
}

func (ao *AllOne) rightAdjust(n *LinkedListNode) {
	for cur := n.Next; cur != nil; cur = cur.Next {
		if cur.Val > n.Val {
			ao.list.Unlink(n)
			ao.list.InsertBefore(cur, n)
			break
		} else if cur == ao.list.tail {
			ao.list.Unlink(n)
			ao.list.RPush(n)
			break
		}
	}
}

/** Decrements an existing key by 1. If Key's value is 1, remove it from the data structure. */
func (ao *AllOne) Dec(key string) {
	if n, ok := ao.data[key]; ok {
		if n.Val == 1 {
			delete(ao.data, key)
			ao.list.Unlink(n)
		} else {
			n.Val--
			ao.leftAdjust(n)
		}
	}
}

func (ao *AllOne) leftAdjust(n *LinkedListNode) {
	for cur := n.Next; cur != nil; cur = cur.Prev {
		if cur.Val < n.Val {
			ao.list.Unlink(n)
			ao.list.InsertAfter(cur, n)
			break
		} else if cur == ao.list.head {
			ao.list.Unlink(n)
			ao.list.LPush(n)
			break
		}
	}
}

/** Returns one of the keys with maximal value. */
func (ao *AllOne) GetMaxKey() string {
	if len(ao.data) == 0 {
		return ""
	}

	return ao.list.tail.Key
}

/** Returns one of the keys with Minimal value. */
func (ao *AllOne) GetMinKey() string {
	if len(ao.data) == 0 {
		return ""
	}

	return ao.list.head.Key
}

// 构造双向链表，加上leftAdjust和rightAdjust提供升序链表
type LinkedList struct {
	head *LinkedListNode
	tail *LinkedListNode
	size int
}

type LinkedListNode struct {
	Key  string
	Val  int
	Next *LinkedListNode
	Prev *LinkedListNode
}

func NewLinkedList() *LinkedList {
	return &LinkedList{}
}

func (ll *LinkedList) LPush(n *LinkedListNode) {
	if ll.head == nil {
		ll.head, ll.tail = n, n
	} else {
		n.Next = ll.head
		ll.head.Prev = n
		ll.head = n
	}

	ll.size++
}

func (ll *LinkedList) RPush(n *LinkedListNode) {
	if ll.head == nil {
		ll.head, ll.tail = n, n
	} else {
		ll.tail.Next = n
		n.Prev = ll.tail
		ll.tail = n
	}

	ll.size++
}

func (ll *LinkedList) LPop() *LinkedListNode {
	if ll.size == 0 {
		return nil
	}

	if ll.head == ll.tail {
		ll.tail = nil
	}

	n := ll.head
	ll.head = n.Next
	n.Next = nil
	ll.size--
	return n
}

func (ll *LinkedList) RPop() *LinkedListNode {
	if ll.size == 0 {
		return nil
	}

	if ll.head == ll.tail {
		ll.head = nil
	}

	n := ll.tail
	ll.tail = n.Prev

	n.Prev = nil

	ll.size--
	return n
}

func (ll *LinkedList) Unlink(n *LinkedListNode) {
	if n == ll.head {
		ll.LPop()
		return
	}

	if n == ll.tail {
		ll.RPop()
		return
	}

	n.Prev.Next = n.Next
	n.Next.Prev = n.Prev

	n.Prev = nil
	n.Next = nil

	ll.size--
}

func (ll *LinkedList) InsertBefore(pos, n *LinkedListNode) {
	if pos == ll.head {
		ll.LPush(n)
		return
	}

	prev := pos.Prev
	prev.Next = n
	n.Prev = prev
	n.Next = pos
	pos.Prev = n

	ll.size++
}

func (ll *LinkedList) InsertAfter(pos, n *LinkedListNode) {
	if pos == ll.tail {
		ll.RPush(n)
		return
	}

	next := pos.Next
	pos.Next = n
	n.Prev = pos
	n.Next = next
	next.Prev = n

	ll.size++
}

/**
 * Your AllOne object will be instantiated and called as such:
 * obj := Constructor();
 * obj.Inc(key);
 * obj.Dec(key);
 * param_3 := obj.GetMaxKey();
 * param_4 := obj.GetMinKey();
 */
